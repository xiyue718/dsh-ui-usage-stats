...
4. The page shows the DeepSeek account balance at the top.
5. Wait for the statistics to load, then view usage and cost by workspace, session, period, and model.
6. Use the "Total" at the top of the page to see the summary across all workspaces.
7. Check or uncheck session types on the far right of the refresh row; the page shows only the selected types.
8. Click "Refresh" to re-check the revision of all sessions and recalculate only changed ones, while also refreshing the balance.

## Features

- Adds a Settings entry: Settings → Usage Stats.
- Balance display at the top of the page: queries the DeepSeek `GET /user/balance` endpoint and shows currency, total balance, granted balance, and topped-up balance.
- Displayed fields:
  - Workspace path
  - Session ID
  - Session title
...
Based on DeepSeek's official peak/off-peak pricing. Peak hours are Beijing time 9:00-12:00 and 14:00-18:00; all other hours are off-peak. Saturdays and Sundays use off-peak pricing all day, with no peak pricing.
...
| deepseek-v4-flash-vision-exp | Off-peak | 0.05 CNY | 1.5 CNY | 4.5 CNY |
| deepseek-v4-flash-vision-exp | Peak | 0.10 CNY | 3.0 CNY | 9.0 CNY |
...
On the host side, it reads session logs through `sessionPersistence.readRaw` or `sessionQuery.readSession`, focusing on the model `usage` data in `assistant/message` events. It aggregates cache miss, cache hit, and output tokens by model and peak/off-peak period, then calculates cost using DeepSeek's official pricing. Peak hours are Beijing time 9:00-12:00 and 14:00-18:00; Saturdays and Sundays are billed at off-peak prices all day. Fork sessions skip chat records inherited from the original session using `header.seedLength` to avoid double counting.

Balance lookup uses DeepSeek's `GET https://api.deepseek.com/user/balance` endpoint. The API key is read from the project credentials service under `DEEPSEEK_API_KEY`, and the `balance_infos` response is mapped to the balance card at the top of the page.
...
On the client side, it renders the Usage Stats page, calls `/api/stats` for data, displays it level by level by workspace, session, period, and model, and provides collapse, totals, session type filtering, and refresh operations. The balance card calls `/api/balance`. The percentage correction in the chat window is calculated by a plugin-side component from the tokenUsage projection and displayed with two decimal places.
